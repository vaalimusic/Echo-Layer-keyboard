...
public final class RichInputConnection {
    ...
    public String getTextBeforeCursorCache() {
        return mTextBeforeCursor;
    }

    public String getTextAfterCursorCache() {
        return mTextAfterCursor;
    }

    public String getSelectedTextCache() {
        return mTextSelection;
    }

    public int getCachedTextStart() {
        if (!hasCursorPosition()) {
            return INVALID_CURSOR_POSITION;
        }
        return mExpectedSelStart - mTextBeforeCursor.length();
    }
    ...
    public boolean replaceTextRange(final int start, final int end, final CharSequence text) {
        mIC = mLatinIME.getCurrentInputConnection();
        if (!isConnected() || !hasCursorPosition() || start < 0 || end < start) {
            return false;
        }
        beginBatchEdit();
        try {
            ...
            if (end > start) {
                ...
                mIC.deleteSurroundingText(0, end - start);
            }
            commitText(text, 1);
            return true;
        } finally {
